...
from vicedtools.scripts.config import (parent_mail_merge_csv,
                                        compass_authenticator,
                                        compass_school_code)


def main():
    folder = os.path.dirname(parent_mail_merge_csv)
    if not os.path.exists(folder):
        os.makedirs(folder)

    s = CompassSession(compass_school_code, compass_authenticator)
    s.export_parent_mailmerge(file_name=parent_mail_merge_csv)
