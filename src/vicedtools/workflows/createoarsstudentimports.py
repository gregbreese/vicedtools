...
def create_oars_student_imports(oars_candidates_json: str,
                                student_details_csv: str,
                                enrolment_details_dir: str,
                                enrolment_cycles: list[str],
                                tag_to_keep_patterns: list[str],
                                oars_dir: str,
                                enrolment_class_pattern: str,
                                pat_most_recent_csv: str):
    ...
    student_details_df = student_details_df.loc[student_details_df["Status"] == "Active"].copy()
    ...
    if enrolment_cycles:
        for cycle in enrolment_cycles:
            enrolment_csv = os.path.join(enrolment_details_dir, f"{cycle} enrolments.csv")
            temp_df = pd.read_csv(enrolment_csv)
            enrolment_details_df = pd.concat([enrolment_details_df,temp_df], ignore_index=True)
        ...
        enrolment_details_df['enrolment length'] = enrolment_details_df['fs'] - enrolment_details_df['ss']
        enrolment_details_df = enrolment_details_df.loc[enrolment_details_df["enrolment length"] > timedelta(days=30)].copy()        
        enrolment_details_df['to tag'] = enrolment_details_df['name'].apply(lambda x: class_selector(x, enrolment_class_pattern))
        enrolment_details_df.dropna(subset=["to tag"], inplace=True)

        enrolment_details_df['Tags'] = enrolment_details_df['name'] + " " + enrolment_details_df["teacherCode"] + " " + enrolment_details_df["ss"].dt.year.astype(str)
        ...
        for candidate in candidates:
            for tag in candidate['tags']:
                    if pattern in tag['name']:
                        tags_to_keep.append({"Username": candidate["username"], "Tags":tag["name"]})
    ...
    pat_results_df["Maths Completed"] = pd.to_datetime(pat_results_df["Maths Completed"])
    recent_maths = pat_results_df.loc[(today - pat_results_df["Maths Completed"]) < timedelta(days=90)]['Username']
    maths_allocations = student_details_df[["Username"]].copy()
    maths_allocations["Tags"] = "Maths adaptive"
    maths_allocations = maths_allocations.loc[~maths_allocations["Username"].isin(recent_maths)].copy()
    pat_results_df["Reading Completed"] = pd.to_datetime(pat_results_df["Reading Completed"])
    recent_reading = pat_results_df.loc[(today - pat_results_df["Reading Completed"]) < timedelta(days=90)]['Username']
    reading_allocations = student_details_df[["Username"]].copy()
    reading_allocations["Tags"] = "Reading adaptive"
    reading_allocations = reading_allocations.loc[~reading_allocations["Username"].isin(recent_reading)].copy()

    # combine tags and assign to students
    tags = pd.concat([enrolment_tags, tags_to_keep, maths_allocations, reading_allocations])
    ...
    for k,g in update_df.groupby(np.arange(len(update_df))//n_rows):
        update_students_xlsx = os.path.join(oars_dir, f"{date_today} update students {(k+1)}.xlsx")
...
if __name__ == "__main__":
    from config import (student_details_csv, oars_candidates_json,
                        enrolment_details_dir, oars_dir,
                        pat_most_recent_csv, oars_tag_enrolment_cycles,
                        oars_tag_enrolment_class_pattern,
                        oars_tag_to_keep_patterns)

    create_oars_student_imports(oars_candidates_json,
                                student_details_csv,
